  ...
  return (
    <div className="py-4 px-5 my-5 relative group overflow-hidden rounded-xl shadow-lg flex  items-center">
      <ListItme
        ...
        className="text-primarytextcolor/90 z-10 transition-all duration-700 font-semibold group-hover:text-secondarylightblue"
      >
        {data}
      </ListItme>
      <Div
        animationDelay={animationDelay}
        className="absolute left-0 h-full transition-all duration-700 w-2 group-hover:w-full text-secondarylightblue bg-primaryblue"
      />
    </div>
  );
};
